...
def display_particle_crop(pidx, p):
    ...
    try:
        ...
        # ONLY SHOW INTERACTIVE BUTTONS IF NOT DELETED
        if not p.get("deleted"):
            ...
            # For deleted particles, just generate a widget ID so View Full button has unique key
            if "widget_counter" not in st.session_state:
                st.session_state.widget_counter = 0
            st.session_state.widget_counter += 1
            widget_id = st.session_state.widget_counter
        ...
        if st.button("🔍", key=f"view_{widget_id}"):
            ...
            if model is None:
                ...
                try:
                    ...
                    for tile_idx, tile_meta in enumerate(st.session_state.tile_metadata):
                        filename = tile_meta.get('source_file') or tile_meta.get('filename')
                        if not filename:
                            st.warning(f"Tile {tile_idx} has no source_file or filename")
                            continue
                        try:
